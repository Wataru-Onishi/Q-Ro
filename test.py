...
goal_current_for_cieling_mA = 12  # in mA
goal_current_for_wall_mA = 3
        ...
        for event in pygame.event.get():
            if event.type == JOYBUTTONDOWN:
                if joystick.get_button(0):  # X button
                    ...
                    print(f"ID 7: {goal_current_for_wall_mA}mA current set.")
                    ...
                    set_goal_velocity(DXL_ID_2, 0)  # Stop motor 5
                    set_goal_velocity(DXL_ID_3, 0)  # Stop motor 6
                ...
                if joystick.get_hat(0) == (0, 1):  # D-pad Up
                    ...
                    set_operating_mode(DXL_ID_2, VELOCITY_CONTROL_MODE)
                    set_operating_mode(DXL_ID_3, VELOCITY_CONTROL_MODE)
                    set_goal_velocity(DXL_ID_2, turning_velocity)  # Motor 5 turns backward
                    set_goal_velocity(DXL_ID_3, turning_velocity)  # Motor 6 turns forward
                    print("Turning right with Motors 5 and 6.")
                elif joystick.get_hat(0) == (-1, 0):  # D-pad Left
                    set_operating_mode(DXL_ID_2, VELOCITY_CONTROL_MODE)
                    set_operating_mode(DXL_ID_3, VELOCITY_CONTROL_MODE)
                    set_goal_velocity(DXL_ID_2, -turning_velocity)  # Motor 5 turns forward
                    set_goal_velocity(DXL_ID_3, -turning_velocity)  # Motor 6 turns backward
